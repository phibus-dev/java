...
public class KafkaProfileService {
    ...
    public KafkaProfileService(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public Profile create(ProfileRequest request) {
        validate(request); UUID id = UUID.randomUUID(); if (request.defaultProfile()) clearDefault();
        jdbc.update("""
                ...
                blankToNull(request.caCertificatePath()), blankToNull(request.defaultTopic()), defaultValue(request.clientIdPrefix(), "evo-snt"), request.defaultProfile());
        return get(id);
    }

    @Transactional
    public Profile update(UUID id, ProfileRequest request) {
        get(id); validate(request); if (request.defaultProfile()) clearDefault();
        jdbc.update("""
                UPDATE kafka_profile SET name=?, bootstrap_servers=?, security_protocol=?, sasl_mechanism=?, username=?,
                       ...
                       default_topic=?, client_id_prefix=?, is_default=?, updated_at=CURRENT_TIMESTAMP WHERE id=?
                """, request.name().trim(), request.bootstrapServers().trim(), normalizedProtocol(request.securityProtocol()),
                blankToNull(request.saslMechanism()), blankToNull(request.username()), normalizedSource(request.credentialsSource()),
                blankToNull(request.vaultSecretPath()), defaultValue(request.passwordField(), "password"), blankToNull(request.passwordEnv()),
                blankToNull(request.caCertificatePath()), blankToNull(request.defaultTopic()), defaultValue(request.clientIdPrefix(), "evo-snt"), request.defaultProfile(), id);
        return get(id);
    }
    ...
    private Profile map(ResultSet rs, int row) throws SQLException {
        return new Profile(rs.getObject("id", UUID.class), rs.getString("name"), rs.getString("bootstrap_servers"), rs.getString("security_protocol"),
                ...
                rs.getString("password_field"), rs.getString("password_env"), rs.getString("ca_certificate_path"), rs.getString("default_topic"),
    ...
    private static String normalizedSource(String value){String source=defaultValue(value,"VAULT").toUpperCase();if(!List.of("VAULT","ENVIRONMENT","NONE").contains(source))throw new IllegalArgumentException("Unsupported Kafka credentials source: "+source);return source;}
