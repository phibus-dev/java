...
public class ClickHouseReplicationObservabilityService {
    ...
    public ClickHouseReplicationObservabilityService(ClickHouseProfileService profiles,
                                                      ClickHouseReplicationSnapshotStore history,
                                                      ClickHouseReplicationMetrics metrics) {
    ...
    private static List<Replica> replicas(Connection c, String db) throws SQLException {
        ...
        try (PreparedStatement s = c.prepareStatement(sql)) {
            ...
            try (ResultSet r = s.executeQuery()) {
                List<Replica> out = new ArrayList<>();
    ...
    private static List<PartSummary> parts(Connection c, String db) throws SQLException {
        ...
        try (PreparedStatement s = c.prepareStatement(sql)) {
            s.setString(1, db);
            try (ResultSet r = s.executeQuery()) {
                List<PartSummary> out = new ArrayList<>();
    ...
    static Health health(List<Replica> replicas, List<QueueItem> queue, List<Mutation> mutations) {
