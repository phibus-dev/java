(() => {
  ...
  function reset(){ $('id').value=''; selectedProfileId=''; $('formTitle').textContent='Новый профиль'; fields.forEach(k=>$(k).value=''); $('securityProtocol').value='PLAINTEXT'; $('credentialsSource').value='NONE'; $('passwordField').value='password'; $('clientIdPrefix').value='evo-snt'; $('defaultProfile').checked=false; $('diagnostic').textContent=''; }
  function edit(p){ if(!p)return; $('id').value=p.id; selectedProfileId=p.id; $('formTitle').textContent=`Редактирование: ${p.name}`; fields.forEach(k=>$(k).value=p[k]??''); $('defaultProfile').checked=!!p.defaultProfile; window.scrollTo({top:0,behavior:'smooth'}); }
  ...
  async function save(){ try{ const id=$('id').value; const saved=await request(id?`/api/kafka/profiles/${id}`:'/api/kafka/profiles',{method:id?'PUT':'POST',body:JSON.stringify(payload())}); selectedProfileId=saved?.id||id||selectedProfileId; EvoUI?.notify('Профиль Kafka сохранён','success'); await load(); const savedProfile=items.find(p=>p.id===selectedProfileId); if(savedProfile){edit(savedProfile);} }catch(e){EvoUI?.notify(e.message,'error');} }
  async function check(){ try{ const id=$('id').value||selectedProfileId; if(!id) throw new Error('Выберите сохранённый профиль кнопкой «Изменить» или сохраните новый профиль'); $('diagnostic').textContent='Проверка…'; const v=await request(`/api/kafka/profiles/${id}/check`,{method:'POST'}); $('diagnostic').textContent=JSON.stringify(v,null,2); }catch(e){$('diagnostic').textContent=e.message;EvoUI?.notify(e.message,'error');} }
